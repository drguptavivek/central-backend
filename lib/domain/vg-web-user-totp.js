...
const DEFAULT_TOTP_MAX_FAILURES = 6;
...
const isLockedOut = async (container, actorId, ip) => {
  ...
  const { maxFailures, windowMinutes, lockDurationMinutes } = await getRateLimitConfig(VgWebUserTotp);

  const recentFailures = await VgWebUserTotp.getRecentFailures(actorId, ip, windowMinutes);
  if (recentFailures < maxFailures) return false;
...
const recordFailure = async (container, actorId, ip, attemptType) => {
  ...
  if (recentFailures >= maxFailures) {
    ...
    const error = new Problem(429.8, 'Too many failed verification attempts. Please try again later.', {
      retryAfterSeconds: lockDurationMinutes * 60
    });
    throw error;
  }
};
