...
const recordTelemetry = async ({ VgTelemetry }, actorId, payload) => {
  ...
  const payloadAppUserId = parseOptionalInt(payload.appUserId, 'appUserId');
  if (payloadAppUserId != null && payloadAppUserId !== actorId)
    throw Problem.user.invalidEntity({ reason: 'appUserId must match the authenticated app user.' });
  ...
  const deduped = (() => {
    ...
    for (let i = events.length - 1; i >= 0; i -= 1) {
      ...
      if (seen.has(entry.clientEventId)) continue;
  ...
  const results = [];
  for (const { clientEventId, event } of deduped) {
    // The database upserts must remain ordered to preserve the client's event order.
