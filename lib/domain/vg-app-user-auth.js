...
const normalizeIp = (ip, payload) => {
  ...
  if (typeof ip !== 'string') throw Problem.user.missingParameters({ expected: ['ip'], got: payload });
  return ip.trim();
};
...
const login = async (container, { username, password, ip, userAgent, deviceId, comments, projectId }) => {
  ...
  if (projectId != null && Number(projectId) !== Number(recordProjectId)) {
    await logAudit(container, null, 'vg.app_user.login.failure', null, { username, ip, reason: 'project_mismatch', projectId });
    throw Problem.user.authenticationFailed();
  }
  ...
  await logAudit(container, actor ?? record.actorId, 'vg.app_user.login.success', actor ?? record.actorId, { username, ip, projectId: projectIdValue });
...
const revokeSessions = async (container, actorId, currentSession, currentOnly = false, deviceId = null) => {
  ...
  const actor = record?.aux?.actor?.orNull?.() ?? record?.aux?.actor ?? null;
  if (currentOnly) {
    if (currentSession != null) await Sessions.terminate(currentSession);
  } else {
...
const revokeSessionById = async (container, session, revokedBy) => {
  ...
  await VgAppUserAuth.expireSessionById(session.id, new Date());
  if (session.token != null) await Sessions.terminate({ token: session.token });
  await logAudit(container, revokedBy ?? null, 'vg.app_user.sessions.revoke', session.actorId, {
