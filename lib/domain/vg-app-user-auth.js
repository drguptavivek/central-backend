...
const recordFailure = async (VgAppUserAuth, username, ip, projectId) => {
...
const login = async (container, { username, password, ip, userAgent, deviceId, comments, projectId }) => {
  ...
  if (auth.isEmpty() || auth.get().vg_active !== true) {
    await recordFailure(VgAppUserAuth, cleanedUsername, ip, projectId);
  ...
  if (!ok) {
    await recordFailure(VgAppUserAuth, cleanedUsername, ip, projectIdValue);
