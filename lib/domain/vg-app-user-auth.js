...
const DEFAULT_LOCK_MAX_FAILURES = 5;
const DEFAULT_LOCK_WINDOW_MINUTES = 5;
const DEFAULT_LOCK_DURATION_MINUTES = 10;
const USERNAME_PATTERN = /^[a-z0-9._-]+$/i;
const NON_ASCII = /[^\x20-\x7E]/;

const forApi = (fieldKey) => ({
  ...fieldKey.forApi(),
  token: null,
  session: undefined
});

const forCreateApi = (fieldKey) => ({
  ...forApi(fieldKey),
  token: undefined
});

const projectForAppUser = async ({ FieldKeys, Projects }, auth, projectId) => {
  const actor = auth.actor.orNull();
  if (actor?.type !== 'field_key') return undefined;

  const fieldKey = await FieldKeys.getByProjectAndActorId(projectId, actor.id);
  if (fieldKey.isEmpty()) throw Problem.user.insufficientRights();
  return Projects.getById(projectId, QueryOptions.none).then(getOrNotFound);
};

const setProperties = async (ActorProperties, project, fieldKey, properties) => {
  if (properties == null) return;
  const knownProperties = await ActorProperties.getAllForProject(project.id);
  const extracted = extractActorProperties(properties, knownProperties.map((p) => p.name));
  await ActorProperties.setValuesForActor(project.id, fieldKey, extracted);
};
...
const getLock = async (VgAppUserAuth, username, ip) => {
  const lockedUntil = await VgAppUserAuth.getActiveLockout(username, ip);
...
const recordFailure = async (container, username, ip, projectId) => {
  const { VgAppUserAuth } = container;
  await VgAppUserAuth.recordAttempt(username, ip, false);
  const { maxFailures, windowMinutes, durationMinutes } = await getLockConfig(VgAppUserAuth, projectId);
  const status = await VgAppUserAuth.getLockStatus(username, ip, windowMinutes);
  if (Number(status.recent_failures || 0) >= maxFailures)
    await VgAppUserAuth.recordLockout(username, ip, durationMinutes);
};

const ensurePasswordPolicy = (password) => {
  if (!validateVgPassword(password))
    throw Problem.user.passwordTooWeak({
      warning: 'The password does not meet the required policy requirements.',
      suggestions: []
    });
};
...
const createAppUser = async (container, project, payload, createdBy) => {
  ...
  const fk = await Actors.createSubtype(
    FieldKey.fromApi({ displayName: cleanedName })
      .with({ createdBy: createdBy ?? null, projectId: project.id }),
    project
  );
...
const updateAppUser = async (container, actorId, projectId, payload) => {
  const { VgAppUserAuth } = container;
  // Upstream actor CRUD calls this field displayName; keep fullName as the
  // VG-facing alias without widening the update to auth fields.
  const fullName = payload.fullName ?? payload.displayName;
  const { phone } = payload;
  ...
  if (updates.fullName != null)
    await VgAppUserAuth.updateDisplayName(actorId, updates.fullName);
...
const login = async (container, { username, password, ip, userAgent, deviceId, comments, projectId }) => {
  ...
  if (auth.isEmpty() || auth.get().vg_active !== true) {
    await recordFailure(container, cleanedUsername, ip, projectId);
  ...
  if (!ok) {
    await recordFailure(container, cleanedUsername, ip, projectIdValue);
  ...
  const ttlDays = await VgAppUserAuth.getSettingWithProjectOverride(
    projectIdValue,
    'vg_app_user_session_ttl_days',
    3
  );
  const sessionCap = await VgAppUserAuth.getSettingWithProjectOverride(
    projectIdValue,
  ...
  await VgAppUserAuth.trimSessionsByActorId(record.actorId, sessionCap);
