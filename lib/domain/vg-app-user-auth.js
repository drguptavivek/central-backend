...
const createAppUser = async (container, project, payload, createdBy) => {
  ...
  if (isBlank(username)) throw Problem.user.missingParameters({ expected: ['username'], got: payload });
  ...
  await VgAppUserAuth.insertAuth({ actorId: fk.actor.id, username, passwordHash: hash, phone, active });
  return fk;
};
...
const login = async (container, { username, password, ip }) => {
  ...
  return { token: session.token, projectId: record.fieldKey?.projectId || record.projectId };
};
