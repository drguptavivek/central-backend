...
const phony = (request) => {
  ...
  // Avoid wrapping /sessions in a transaction so failed login audits persist
  const requestPath = (request.path ?? request.url ?? request.originalUrl ?? '').split('?')[0];
  const routePath = requestPath.replace(/^\/v\d+(?=\/)/, '');
  // App-user login has the same requirement: failed attempts and lockouts are
  // durable security state, so do not roll them back with the failed request.
  // Match the complete route after removing the API version, including exactly
  // one project-id path segment, so adjacent app-user routes remain transactional.
  const appUserLoginPath = /^\/projects\/[^/]+\/app-users\/login$/;
  if (request.method === 'POST' && (routePath === '/sessions' || appUserLoginPath.test(routePath))) return true;
  return false;
};
...
const defaultErrorWriter = (error, request, response) => {
  ...
  if (error?.isProblem === true) {
    if (error.retryAfterSeconds != null && !response.headersSent)
      response.setHeader('Retry-After', String(error.retryAfterSeconds));
    if (error.loginAttemptsRemaining != null && !response.headersSent)
      response.setHeader('X-Login-Attempts-Remaining', String(error.loginAttemptsRemaining));
