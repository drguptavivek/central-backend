...
// VG: Helper to extract client IP from request
...
const authHandler = ({ Sessions, Users, Auth, VgWebUserTotp, VgUserIpWhitelist }, context, request) => {
  // VG: Check TOTP verification for cookie auth (web users)
  const checkTotpVerification = (session, cxt) => {
    // Skip TOTP check for certain endpoints (setup, enable, verify, etc.)
    ...
    const skipTotpPaths = [
      ...
      /\/users\/[^\/]+\/totp\/status/,
      /\/sessions\/totp-verify/
    ];
    if (skipTotpPaths.some(re => re.test(path))) {
      return Promise.resolve(cxt);
    }

    // If session is not TOTP verified, check if user has TOTP enabled
    if (session.totp_verified === false) {
      return VgWebUserTotp.getByActorId(session.actor.id)
        .then((maybeRecord) => {
          if (maybeRecord.isDefined() && maybeRecord.get().totp_enabled) {
            // User has TOTP enabled but session is not verified
  ...
  // VG: Check IP whitelist for bearer token auth
  const checkIpWhitelist = (session, cxt) => {
    // Only check for user (web user) bearer tokens, not field keys or public links
    ...
    return VgUserIpWhitelist.hasAnyEntries(session.actor.id)
      .then((hasEntries) => {
        if (!hasEntries) {
          return cxt; // No whitelist = allow all
        }
  ...
  const authBySessionToken = (token, isCookie = false) => Sessions.getByBearerToken(token)
    .then(getOrReject(Problem.user.authenticationFailed()))
    .then(session => context.with({ auth: Auth.by(session) }))
    .then((cxt) => {
      // VG: Apply TOTP check for cookie auth
      if (isCookie) {
        return checkTotpVerification(cxt.auth.session.get(), cxt);
      }
      // VG: Apply IP whitelist check for bearer token auth (non-cookie)
  ...
  // Standard Bearer token auth:
    ...
    return authBySessionToken(authHeader.substring(7), false);
  ...
  // Cookie Auth:
    ...
    // actually try to authenticate with it (VG: pass isCookie=true)
    return authBySessionToken(token, true)
