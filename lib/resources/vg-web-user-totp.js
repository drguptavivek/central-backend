...
module.exports = (service, endpoint) => {
  ...
  // Frontend sends session token via Authorization: Bearer header
  service.post('/sessions/totp-verify', endpoint(async (container, { auth, body }, request) => {
    ...
    // Frontend sends this via Authorization: Bearer header
    const session = auth.session.orElseGet(() => { throw Problem.user.authenticationFailed(); });
    ...
    return (_, response) => {
      ...
      // VG: Return session data so frontend can set session.data and proceed with login
      // Need to re-fetch session to get updated expiresAt (was extended in markTotpVerified)
  ...
  // Dismiss TOTP enrollment prompt
  // Body: { remindAfterDays: number | null }
  service.post('/users/:id/totp/dismiss-enrollment-prompt', endpoint(async (container, { auth, params, body }) => {
    const { Users, VgWebUserTotp } = container;
    const actorId = parseInt(params.id, 10);
    const { remindAfterDays } = body;

    // Validate remindAfterDays
    if (remindAfterDays !== null && (typeof remindAfterDays !== 'number' || remindAfterDays < 1 || remindAfterDays > 365)) {
      throw Problem.user.unexpectedValue({ field: 'remindAfterDays', value: remindAfterDays, reason: 'Must be a number between 1 and 365, or null.' });
    }
