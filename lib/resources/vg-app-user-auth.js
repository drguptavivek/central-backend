...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  service.post('/projects/:projectId/app-users/login', anonymousEndpoint(
    ({ Projects, VgAppUserAuth, Sessions }, { body, params }, request) => {
      ...
      return Projects.getById(params.projectId)
        .then(getOrNotFound)
        .then(() => vgAuth.login({ VgAppUserAuth, Sessions }, { username, password, ip: request.ip }));
    }
  ));

  // Change password (self).
  service.post('/projects/:projectId/app-users/:id/password/change', endpoint(
    ({ Projects, FieldKeys, VgAppUserAuth, Sessions }, { auth, body, params }) =>
      Projects.getById(params.projectId)
        ...
        .then((fk) => {
          ...
          return vgAuth.changePassword({ VgAppUserAuth, Sessions }, fk.actor.id, { oldPassword, newPassword })
            .then(() => success);
        })
  ));
    ...
    ({ Projects, FieldKeys, VgAppUserAuth, Sessions }, { auth, body, params }) =>
      Projects.getById(params.projectId)
        ...
        .then((fk) => {
          ...
          return vgAuth.resetPassword({ VgAppUserAuth, Sessions }, fk.actor.id, { newPassword })
            .then(() => success);
        })
  ));
  ...
  service.post('/projects/:projectId/app-users/:id/revoke', endpoint(
    ({ Projects, FieldKeys, Sessions }, { auth, params }) =>
      Projects.getById(params.projectId)
        .then(getOrNotFound)
        .then((project) => FieldKeys.getByProjectAndActorId(project.id, params.id))
        .then(getOrNotFound)
        .then((fk) => {
          ...
          return vgAuth.revokeSessions({ Sessions }, fk.actor.id, currentToken)
            .then(() => success);
        })
  ));
    ...
    ({ Projects, FieldKeys, Sessions }, { auth, params }) =>
      Projects.getById(params.projectId)
        ...
        .then((project) => auth.canOrReject('field_key.delete', project)
          .then(() => FieldKeys.getByProjectAndActorId(project.id, params.id)))
        .then(getOrNotFound)
        .then((fk) => vgAuth.revokeSessions({ Sessions }, fk.actor.id, null)
          .then(() => success))
  ));
    ...
    ({ Projects, FieldKeys, VgAppUserAuth, Sessions }, { auth, body, params }) =>
      Projects.getById(params.projectId)
        ...
        .then((fk) => {
          ...
          return vgAuth.setActive({ VgAppUserAuth, Sessions }, fk.actor.id, active)
            .then(() => success);
        })
  ));
};
