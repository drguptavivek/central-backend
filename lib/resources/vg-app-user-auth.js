      ...
      Projects.getById(params.projectId)
        ...
        .then((fk) => {
          ...
          if (isBlank(oldPassword) || isBlank(newPassword))
            return Problem.user.missingParameters({ expected: ['oldPassword', 'newPassword'], got: payload });
      ...
      Projects.getById(params.projectId)
        ...
        .then((fk) => {
          ...
          if (isBlank(newPassword))
            return Problem.user.missingParameters({ expected: ['newPassword'], got: payload });
  ...
  service.put('/system/settings', endpoint(
    ({ VgAppUserAuth }, { auth, body }) =>
      auth.canOrReject('config.set', Config.species)
        .then(() => {
          const { vg_app_user_session_ttl_days, vg_app_user_session_cap, admin_pw } = body;
