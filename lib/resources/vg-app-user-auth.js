...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  service.post('/projects/:projectId/app-users/login', anonymousEndpoint(
    ({ Projects, VgAppUserAuth, VgAppUserIpRateLimit, Sessions, Audits }, { body, params }, request) => {
      ...
      const cleanedUsername = (typeof username === 'string' ? username.trim().toLowerCase() : username);
    ...
    ({ VgAppUserAuth }, { auth, body }) =>
      auth.canOrReject('config.set', Config.species)
        .then(() => {
          ...
          if (vg_app_user_session_ttl_days === undefined && vg_app_user_session_cap === undefined &&
              vg_app_user_ip_max_failures === undefined && vg_app_user_ip_window_minutes === undefined &&
              vg_app_user_ip_lock_duration_minutes === undefined && admin_pw === undefined) {
          ...
          const ipMaxFailures = parsePositiveInt(vg_app_user_ip_max_failures, 'vg_app_user_ip_max_failures');
          const ipWindowMinutes = parsePositiveInt(vg_app_user_ip_window_minutes, 'vg_app_user_ip_window_minutes');
          const ipLockDurationMinutes = parsePositiveInt(vg_app_user_ip_lock_duration_minutes, 'vg_app_user_ip_lock_duration_minutes');
          ...
          if (admin_pw !== undefined) {
            ...
            if (admin_pw.length > ADMIN_PW_MAX_LENGTH) {
          ...
          if (ipMaxFailures != null)
            promises.push(VgAppUserAuth.upsertSetting('vg_app_user_ip_max_failures', ipMaxFailures));
          if (ipWindowMinutes != null)
            promises.push(VgAppUserAuth.upsertSetting('vg_app_user_ip_window_minutes', ipWindowMinutes));
          if (ipLockDurationMinutes != null)
            promises.push(VgAppUserAuth.upsertSetting('vg_app_user_ip_lock_duration_minutes', ipLockDurationMinutes));
          if (admin_pw !== undefined)
      ...
      Projects.getById(params.projectId)
        ...
        .then((project) => auth.canOrReject('project.update', project)
          .then(() => {
            ...
            if (ipMaxFailures != null)
              updates.push(VgAppUserAuth.upsertProjectSetting(project.id, 'vg_app_user_ip_max_failures', ipMaxFailures));
            if (ipWindowMinutes != null)
              updates.push(VgAppUserAuth.upsertProjectSetting(project.id, 'vg_app_user_ip_window_minutes', ipWindowMinutes));
            if (ipLockDurationMinutes != null)
              updates.push(VgAppUserAuth.upsertProjectSetting(project.id, 'vg_app_user_ip_lock_duration_minutes', ipLockDurationMinutes));
            if (admin_pw !== undefined)
              updates.push(VgAppUserAuth.upsertProjectSetting(project.id, 'admin_pw', admin_pw));
