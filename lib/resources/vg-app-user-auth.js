...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  service.post('/projects/:projectId/app-users/login', anonymousEndpoint(
    ({ Projects, VgAppUserAuth, VgAppUserIpRateLimit, Sessions, Audits }, { body, params }, request) => {
      ...
      const recordFailureAndMaybeIpLockout = () => {
        // Check current failure count, domain layer will log the failure
        return Promise.all([
          VgAppUserIpRateLimit.getAppUserLoginFailuresFromIp(normalizedIp, 15),
          VgAppUserIpRateLimit.hasRecentAppUserIpLockout(normalizedIp, 30)
        ])
          .then(([ipCount, isLocked]) => {
            // Lock when count exceeds 20 (allows 20 attempts, locks on 21st)
            // Note: domain layer logs BEFORE this check, so count includes current failure
            if (!isLocked && ipCount > 20) {
              return Audits.log(null, 'vg.app_user.login.ip.lockout', null, {
                ip: normalizedIp,
                durationMinutes: 30
              }).then(() => ({
                ipLocked: true,
                ipRetryAfterSeconds: 30 * 60
              }));
            }
      ...
      return VgAppUserIpRateLimit.isAppUserIpLocked(normalizedIp)
        ...
        .catch((problem) => {
          if (problem?.problemCode === authFailed.problemCode) {
            ...
            return recordFailureAndMaybeIpLockout()
              .then((ipResult) => {
    ...
    ({ VgAppUserAuth }, { auth, body }) =>
      auth.canOrReject('config.set', Config.species)
        .then(() => {
          ...
          if (vg_app_user_session_ttl_days === undefined && vg_app_user_session_cap === undefined && admin_pw === undefined) {
            throw Problem.user.missingParameters({
              expected: ['vg_app_user_session_ttl_days', 'vg_app_user_session_cap', 'admin_pw'],
              got: payload
            });
          }
  ...
  // Project app-user settings (admin_pw, ttl, cap).
