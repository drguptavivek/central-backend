            ...
            return VgWebUserTotp.getByActorId(user.actorId)
              .then((maybeTotpRecord) => (_, response) => {
                if (maybeTotpRecord.isDefined() && maybeTotpRecord.get().totp_enabled) {
                  // Create session but mark it as not TOTP verified
                  return createUserSession({ Audits, Sessions, Users }, headers, user, false)
                    .then((middleware) => {
                      // Execute middleware to set cookies and get session
                      const session = middleware(_, response);
                      // Return session with requireTotp flag
