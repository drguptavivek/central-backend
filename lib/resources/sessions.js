...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  if (!oidc.isEnabled()) {
    service.post('/sessions', anonymousEndpoint(({ Audits, Users, Sessions }, { body, headers }, request) => {
      ...
      const recordFailureAndMaybeLockout = () => {
        ...
        return logFailedLogin()
          .then(() => Promise.all([
            ...
            Audits.hasRecentWebLoginLockout(normalizedEmail, normalizedIp, LOCK_DURATION_MINUTES)
          ]))
          .then(([count, isLocked]) => {
