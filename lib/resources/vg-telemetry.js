...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:projectId/app-users/telemetry', endpoint(
    ({ Projects, FieldKeys, VgTelemetry, Sessions }, { auth, body, params }) => {
      const actor = auth.actor.orNull();
      if (actor == null || actor.type !== 'field_key')
        return Problem.user.insufficientRights();

      const token = auth.session.map((s) => s.token).orNull();
      return Projects.getById(params.projectId)
        ...
        .then(async ({ id, received_at, deviceId }) => {
          // The request has already authenticated; this is a final "am I still valid?"
          // check for clients to detect revocation/expiry races.
          const invalidated = (token == null)
            ? false
            : (await Sessions.getByBearerToken(token)).isEmpty();
          return {
            id,
            appUserId: actor.id,
            deviceId,
            ...
            status: invalidated ? 'invalidated' : 'ok'
          };
        });
    }
  ));
