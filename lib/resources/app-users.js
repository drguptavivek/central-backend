...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:projectId/app-users', endpoint(({ FieldKeys, Projects, VgAppUserAuth, Sessions }, { auth, body, params }) =>
    Projects.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('field_key.create', project)
        .then(() => vgAuth.createAppUser({ FieldKeys, VgAppUserAuth, Sessions }, project, body, auth.actor.map((actor) => actor.id).orNull())))));
