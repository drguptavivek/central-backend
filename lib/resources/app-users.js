...
// The VG auth fields are exposed alongside the normal FieldKey fields, but
// session tokens must never be returned by app-user management endpoints.
...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/app-users', endpoint(({ FieldKeys, Projects }, { auth, params, queryOptions }) =>
    Projects.getById(params.projectId)
      ...
      .then((keys) => keys.map(forApi))));

  service.post('/projects/:projectId/app-users', endpoint(async ({ ActorProperties, FieldKeys, Projects, VgAppUserAuth, Sessions, Audits }, { auth, body, params }) => {
    ...
    const fieldKey = await vgAuth.createAppUser(
      { FieldKeys, VgAppUserAuth, Sessions, Audits },
      project,
      body ?? {},
      auth.actor.map((actor) => actor.id).orNull()
    );
    await setProperties(ActorProperties, project, fieldKey, body?.properties);
    return fieldKey;
  }));
  ...
  service.patch('/projects/:projectId/app-users/:id', endpoint(async ({ ActorProperties, Actors, FieldKeys, Projects, VgAppUserAuth }, { auth, body, params }) => {
    ...
    await setProperties(ActorProperties, project, fk, payload.properties);
