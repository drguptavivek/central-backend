...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:id', endpoint(async ({ Projects, FieldKeys }, { auth, params, query, queryOptions }) => {
    const appUserProject = await vgAuth.projectForAppUser({ FieldKeys, Projects }, auth, params.id);
    if (appUserProject != null) {
      if (queryOptions.extended === true || query.verbs === 'true') {
        const verbs = await auth.verbsOn(appUserProject);
        return Object.assign({ verbs }, appUserProject.forApi());
      }
