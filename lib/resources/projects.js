...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:id', endpoint(async ({ Projects, FieldKeys }, { auth, params, queryOptions }) => {
    // First check if this is an app user accessing their own project
    const actor = auth.actor.orNull();
    if (actor?.type === 'field_key') {
      const fieldKey = await FieldKeys.getByActorId(actor.id);
      if (fieldKey.isEmpty() || fieldKey.get().projectId !== parseInt(params.id, 10))
        return reject(Problem.user.insufficientRights());
      // App users get basic project data only (no extended metadata)
      return Projects.getById(params.id, QueryOptions.none).then(getOrNotFound);
    }

    // For other users, use standard auth check
    const project = await Projects.getById(params.id, queryOptions).then(getOrNotFound);
    await auth.canOrReject('project.read', project);

    if (queryOptions.extended === true) {
      const verbs = await auth.verbsOn(project);
      return Object.assign({ verbs }, project.forApi());
    }
    return project;
  }));
