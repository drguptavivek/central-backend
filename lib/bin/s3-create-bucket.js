...
// Garage implements the S3 API, so retain the Minio Node client as the
// provider-neutral S3 client used by Central.
const s3Client = (() => {
...
s3Client.bucketExists(bucketName)
  .then(exists => {
    if (exists) {
    ...
    return s3Client.makeBucket(bucketName)
      .then(() => log('Bucket created OK.'));
  })
  .catch(err => {
    log('ERROR CREATING S3 BUCKET:', err);
    process.exit(1);
  });
