...
const actionCondition = (action) => {
  if (action === 'nonverbose')
    ...
    return sql`action in ('user.create', 'user.update', 'user.delete', 'user.assignment.create', 'user.assignment.delete', 'user.session.create', 'user.session.create.failure', 'user.session.lockout')`;
...
const webLoginIpFilter = (ip) => (ip == null)
  ? sql`(details->>'ip') IS NULL`
  : sql`details->>'ip'=${ip}`;

const getWebLoginFailureCount = (email, ip, windowMinutes) => ({ maybeOne }) =>
  maybeOne(sql`
    SELECT count(*)::int AS count
    FROM audits
    WHERE action = 'user.session.create.failure'
      AND details->>'email'=${email}
      AND ${webLoginIpFilter(ip)}
      AND "loggedAt" >= now() - (${windowMinutes} * interval '1 minute')
  `).then((opt) => opt.map((row) => row.count).orElse(0));

const hasRecentWebLoginLockout = (email, ip, durationMinutes) => ({ maybeOne }) =>
  maybeOne(sql`
    SELECT COALESCE(
...
const getLatestWebLoginLockoutAt = (email, ip) => ({ maybeOne }) =>
  maybeOne(sql`
...
module.exports = {
  ...
  getWebLoginFailureCount, hasRecentWebLoginLockout, getLatestWebLoginLockoutAt
};
