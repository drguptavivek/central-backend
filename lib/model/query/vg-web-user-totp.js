 ...
 *
 * VG: Service accounts are excluded - they NEVER have mandatory TOTP (use IP whitelist)
 */
const isRoleMandatoryForTotp = (actorId) => ({ maybeOne, all }) =>
  ...
  `).then((maybeUser) => {
    // Service accounts NEVER have mandatory TOTP (regardless of role)
    // They rely on mandatory IP whitelist security instead
 ...
 *
 * VG: Service accounts are excluded from enrollment prompts (they're automated systems)
 */
const shouldPromptEnrollment = (actorId) => ({ maybeOne }) =>
  ...
  maybeOne(sql`
    ...
    WHERE u."actorId" = ${actorId}
  `).then((maybeUser) => {
    // Service accounts NEVER get enrollment prompts (they're automated systems)
    // They rely on IP whitelist security instead
    if (maybeUser.isDefined() && maybeUser.get().is_service_account === true) {
      return false;
    }
