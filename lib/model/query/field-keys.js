...
const _get = extender(FieldKey, Actor, Frame.define('token', readable), Frame.define('active', readable, 'username', readable).into('activeStatus'))(Actor.alias('created_by', 'createdBy'), Frame.define('lastUsed', readable))((fields, extend, options) => sql`
select ${fields} from field_keys
  ...
  left outer join (select "actorId", coalesce(vg_active, true) as active, vg_username as username from vg_field_key_auth) as vg_auth on field_keys."actorId"=vg_auth."actorId"
