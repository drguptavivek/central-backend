...
const _get = extender(FieldKey, Actor, Frame.define('token', readable), Frame.define('active', readable, 'username', readable, 'phone', readable).into('activeStatus'))(Actor.alias('created_by', 'createdBy'), Frame.define('lastUsed', readable), Frame.define('properties', readable))((fields, extend, options) => sql`
...
const getByActorId = (actorId, options = QueryOptions.none) => ({ maybeOne }) =>
  _get(maybeOne, options.withCondition({ 'field_keys.actorId': actorId }));

module.exports = { create, createWithoutSession, getAllForProject, getByProjectAndActorId, getByActorId };
