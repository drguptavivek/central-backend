...
const trimSessionsByActorId = (actorId, limit) => ({ run }) =>
  run(sql`
...
const getAppUserIpMaxFailures = () => getSettingValue('vg_app_user_ip_max_failures', 20);

const getAppUserIpWindowMinutes = () => getSettingValue('vg_app_user_ip_window_minutes', 15);

const getAppUserIpLockDurationMinutes = () => getSettingValue('vg_app_user_ip_lock_duration_minutes', 30);
...
const upsertProjectSetting = (projectId, key, value) => ({ run }) =>
  run(sql`
...
const getSettingWithProjectOverrideRaw = (projectId, key, fallback) => ({ maybeOne }) =>
  maybeOne(sql`
    SELECT COALESCE(
      (SELECT vg_key_value FROM vg_project_settings WHERE "projectId"=${projectId} AND vg_key_name=${key} LIMIT 1),
      (SELECT vg_key_value FROM vg_settings WHERE vg_key_name=${key} LIMIT 1),
  ...
  `).then((opt) => opt.map((row) => row.value).orElse(fallback));
...
const recordSession = ({ token, actorId, ip = null, userAgent = null, deviceId = null, comments = null, expiresAt }) => ({ run }) => {
  ...
  if (expiresAt == null) throw new Error('recordSession: expiresAt is required');
...
const getSessionByToken = (token) => ({ maybeOne }) =>
  maybeOne(sql`
    ...
    JOIN actors a ON a.id = v."actorId"
    JOIN vg_field_key_auth vga ON vga."actorId" = v."actorId"
    WHERE v.token = ${token}
      AND a."deletedAt" IS NULL
      AND vga.vg_active = true
      AND v.expires_at IS NOT NULL
      AND v.expires_at > statement_timestamp()
    LIMIT 1
  `);
...
module.exports = {
  ...
  updatePhone,
  updateDisplayName,
  trimSessionsByActorId,
  getSessionTtlDays,
  getSessionCap,
  getAppUserIpMaxFailures,
  getAppUserIpWindowMinutes,
  getAppUserIpLockDurationMinutes,
  getAdminPw,
  upsertSetting,
  upsertProjectSetting,
  getSettingValue,
  getSettingWithProjectOverride,
  getSettingWithProjectOverrideRaw,
  recordAttempt,
  recordLockout,
  getActiveLockout,
  recordSession,
