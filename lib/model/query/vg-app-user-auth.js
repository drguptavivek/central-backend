...
const upsertSetting = (key, value) => ({ run }) =>
  run(sql`
    INSERT INTO vg_settings (vg_key_name, vg_key_value, updated_at) VALUES (${key}, ${value}, now())
    ON CONFLICT (vg_key_name) DO UPDATE SET vg_key_value=${value}, updated_at=now()
  `);

const upsertProjectSetting = (projectId, key, value) => ({ run }) =>
  run(sql`
    INSERT INTO vg_project_settings ("projectId", vg_key_name, vg_key_value, updated_at)
    VALUES (${projectId}, ${key}, ${value}, now())
    ON CONFLICT ("projectId", vg_key_name) DO UPDATE
      SET vg_key_value = EXCLUDED.vg_key_value, updated_at = now()
  `);
