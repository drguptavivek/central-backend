...
const DEFAULT_IP_WINDOW_MINUTES = 15;
const DEFAULT_IP_LOCK_DURATION_MINUTES = 30;
 ...
 * @param {string|null} ip - IP address to check
 * @param {number} lockDurationMinutes - Lockout duration in minutes (default: DEFAULT_IP_LOCK_DURATION_MINUTES)
...
const recordAppUserFailureAndMaybeLockout = (ip, username, Audits) => ({ maybeOne }) => {
  ...
  return logFailure()
    .then(() => Promise.all([
      getAppUserLoginFailuresFromIp(ip, DEFAULT_IP_WINDOW_MINUTES)({ maybeOne }),
      hasRecentAppUserIpLockout(ip, DEFAULT_IP_LOCK_DURATION_MINUTES)({ maybeOne })
    ]))
    .then(([ipCount, isLocked]) => {
      ...
      if (!isLocked && ipCount > DEFAULT_IP_MAX_FAILURES) {
        return Audits.log(null, 'vg.app_user.login.ip.lockout', null, {
          ip,
          durationMinutes: DEFAULT_IP_LOCK_DURATION_MINUTES
        }).then(() => ({
          ipLocked: true,
          ipRetryAfterSeconds: DEFAULT_IP_LOCK_DURATION_MINUTES * 60
        }));
      }
...
module.exports = {
  ...
  DEFAULT_IP_LOCK_DURATION_MINUTES
};
